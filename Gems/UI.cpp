...
void GameProcess(FieldManager *Matrix, FieldRender *Field, RenderWindow *window)
{
	...
	//Sprite Background
	Texture backTexture;
	backTexture.loadFromFile("resourses/textures/wallpaper1.png");
...
void DeathScreen(FieldManager *Matrix, FieldRender *Field, RenderWindow *window)
{
	...
	death_sound.loadFromFile("resourses/music/Death_sound.ogg");
	Sound sound;
	...
	PlayerScore.setPosition(550, 200);
	PlayerScore.setString(" Score:  " + std::to_string(Matrix->PlayerScore));
	...
	while (window->isOpen() && !Restart)
	{
		...
		if (sf::Mouse::isButtonPressed(sf::Mouse::Left))   
		{
			...
			if (disp_x >= 443 && disp_x <= 827 && disp_y >= 402 && disp_y <= 445)
			{
				Restart = true;
			}
...
void MainMenu(FieldManager *Matrix, FieldRender *Field, RenderWindow *window)
{
	...
	while (window->isOpen())
	{
		...
		if (sf::Mouse::isButtonPressed(sf::Mouse::Left))   
		{
			...
			if (disp_x >= 443 && disp_x <= 839 && disp_y >= 303 && disp_y <= 343)
			{
				break;
			}

			if (disp_x >= 1037 && disp_x <= 1230 && disp_y >= 678 && disp_y <= 720)
			{
				window->close();
			}
		}
